...
| Workflow trigger comment | `github-actions[bot]` | `> **GitHub Actions (automation)** — …` in [cursor-tagging-qa-automation.yml](workflows/cursor-tagging-qa-automation.yml) |
...
5. A cloud agent reads the **issue body** (agent brief + tasks), follows [`.cursor/skills/add-traffic-sign/SKILL.md`](../.cursor/skills/add-traffic-sign/SKILL.md), updates sign config under `packages/traffic-sign-converter/src/data-definitions/DE/`, and **opens a PR**.
...
5. Add GitHub repository secrets:
   ...
   - `CURSOR_TAGGING_QA_WEBHOOK_API_KEY` — token only (no `Bearer ` prefix)
