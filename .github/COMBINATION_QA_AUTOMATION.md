...
| Workflow trigger comment | `github-actions[bot]` | `> **GitHub Actions (automation)** — …` in [cursor-combination-qa-automation.yml](workflows/cursor-combination-qa-automation.yml) |
...
5. A cloud agent reads the **issue body** (agent brief + tasks), follows [`.cursor/skills/fix-sign-combination/SKILL.md`](../.cursor/skills/fix-sign-combination/SKILL.md), updates sign config under `packages/traffic-sign-converter/src/data-definitions/DE/`, and **opens a PR**.
...
3. Prompt the agent to read the webhook JSON (`issueUrl`, `title`, `body`, `issueNumber`), follow `.cursor/skills/fix-sign-combination/SKILL.md`, use `**[Cursor Agent]**` on all GitHub text, and open a PR whose description includes `Closes #<issueNumber>` (auto-closes the issue on merge).
4. Save the automation, copy the webhook URL and API key (`Authorization: Bearer crsr_…`).
5. Add GitHub repository secrets:
   - `CURSOR_COMBINATION_QA_WEBHOOK_URL` — webhook URL from the dashboard
   - `CURSOR_COMBINATION_QA_WEBHOOK_API_KEY` — token only (no `Bearer ` prefix)
